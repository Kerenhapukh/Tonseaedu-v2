...
import { BookOpen, Volume2, PlayCircle, Trophy, User, LogOut, Sparkles } from "lucide-react";
...
export default function Navbar() {
  ...
  return (
    <header className="w-full sticky top-0 z-50">
      <nav className="hidden md:block w-full bg-white/85 backdrop-blur-xl border-b border-slate-200/80 shadow-sm transition-all">
        <div className="max-w-7xl mx-auto px-6 h-20 flex justify-between items-center">
          ...
          <div className="flex items-center gap-10">
            {/* Logo */}
            <Link href="/materi" className="flex items-center gap-3 group">
            ...
            {/* Menu Items */}
            <div className="flex items-center gap-1.5 bg-slate-100/70 p-1.5 rounded-full border border-slate-200/60">
              {menuItems.map((item) => {
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-white/90 backdrop-blur-xl border-t border-slate-200 h-20 shadow-lg">
          ...
          {menuItems.map((item) => {
            ...
            return (
              <Link key={item.href} href={item.href} className={`flex flex-col items-center gap-1 ${isActive ? "text-blue-600" : "text-slate-400"}`}>
                <div className={`p-2 rounded-2xl transition-all ${isActive ? "bg-blue-50 text-blue-600" : ""}`}>
